...
const Comments = () => {
  ...
  const deleteReply = (commentId, replyId) => {
    ...
    const indexOfReply = currentComment.replies.findIndex(
  ...
  const sendReply = () => {
    if (selectedCommentId == parseInt(selectedCommentId)) {
      if (replyValue == '') {
        ...
        const currentCommentedPost = comments.find(
          (comment) => comment.id === selectedCommentId
        );
        const newReply = {
          id:
            currentCommentedPost.id +
            (currentCommentedPost.replies.length + 1) / 10,
          content: replyValue,
          createdAt: 'just now',
      ...
      if (replyValue == '') {
        ...
        const currentCommentedPost = comments.find(
          (comment) => comment.id == parseInt(selectedCommentId)
        );
        const replyOfcurrentCommentedPost =
          currentCommentedPost.replies &&
          currentCommentedPost.replies.find(
            (reply) => reply.id == selectedCommentId
          );
        const replyOfReply = {
          content: replyValue,
          createdAt: 'just now',
        ...
        const index =
          parseInt((selectedCommentId - currentCommentedPost.id) * 10) - 1;
        currentCommentedPost.replies[index].replies = newArray;
        const updateReplyOfReply = [
          ...currentCommentedPost.replies[0].replies,
          replyOfReply,
        ];
  ...
  return (
    <>
              ...
              <div className="received-msg">
                <div className="score-count">
                  <img
                    ...
                    className="minus-img"
                    onClick={() => onDecrement(comment.id)}
                  />
                  <p>{comment.score}</p>
                  <img
                    src="images/icon-plus.svg"
                    className="plus-img"
                    onClick={() => onIcrement(comment.id)}
                  />
                </div>
                  ...
                  <div className="msg-content">
                    {selectedCommentIdFOrEdit == comment.id ? (
                      <>
                        ...
                        <button
                          className="save-btn"
                          onClick={() => saveAfterEdit(comment.id)}
                        >
                          Save
                        </button>
                      </>
                              ...
                              <div className="sent-msg" key={index}>
                                <div className="score-count">
                                  <img
                                    src="images/icon-minus.svg"
                                    className="minus-img"
                                    onClick={() => onDecrement(reply.id)}
                                  />
                                  <p>{reply.score}</p>
                                  <img
                                    src="images/icon-plus.svg"
                                    className="plus-img"
                                    onClick={() => onIcrement(reply.id)}
                                  />
                                </div>
                                <div className="user-content">
                                  <div className="user-content-header">
                                    <div className="user-msg-info">
                                      ...
                                      <p className="msg-time gray-text">
                                        {reply.createdAt}
                                      </p>
                                    </div>
                                    {reply.user.username == 'juliusomo' ? (
                                      ...
                                      <div
                                        className="reply-btn"
                                        onClick={() => handleReplying(reply.id)}
                                      >
                                        <img src="images/icon-reply.svg" />
                                        <p>Reply</p>
                                      </div>
      ...
      {displayDeleteBox && (
        <DeleteComments
          setDisplayDeleteBOx={setDisplayDeleteBOx}
